...
mod tests {
    ...
    /// place of the previous `[0..255]` cyclic fixture because slot
    /// 13 contents now have to parse as a valid TPM2 Import payload
    /// to be accepted by `parse_gks`.
    ...
    fn sample_keys() -> ([u8; AES_KEY_LEN], [u8; SESSION_ID_LEN]) {
        let parsed = parse_gks(SAMPLE_IMPORT_BLOB).expect("sample import blob must parse");
        let session_id = [0xa5u8; SESSION_ID_LEN];
        let aes_key = derive_aes_key(&parsed, &session_id).unwrap();
        (aes_key, session_id)
    }
